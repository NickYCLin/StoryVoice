...
最後更新：2026-08-13（書冊詳情頁新增角色候選偵測，協助匯入後快速找出這一冊有哪些角色）
...
- 書冊詳情頁的角色候選偵測：對已連結的合法正文重用 reporting-clause 規則（與說話者辨識同一套「名字＋說／問／道」判斷），但不限定已知角色，找出對白旁邊出現、尚未註冊的名字並依出現次數排序，附一句原文對白當範例；純提示、不會自動建立角色，也不保存結果（每次即時計算）。已知會漏掉只用代名詞或稱謂互相稱呼的角色，也可能誤收「老師」這類稱謂當作候選。
