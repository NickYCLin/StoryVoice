    ...
    : AuthenticationHandler<AuthenticationSchemeOptions>(schemeOptions, logger, encoder)
{
    ...
    protected override Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        if (!options.Enabled
            ...
            || !hashMatches)
        {
            return Task.FromResult(AuthenticateResult.Fail("External voice API key is invalid."));
        }

        var claims = new[]
        {
            new Claim(ExternalVoiceAuthenticationDefaults.ConsumerKeyIdClaim, keyId),
            new Claim(
                ExternalVoiceAuthenticationDefaults.ExternalOwnerIdClaim,
                consumer.OwnerId.ToString("D")),
            new Claim(
