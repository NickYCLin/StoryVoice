...
export function SeriesCastPanel() {
  ...
  if (state === 'error') {
    return <main className="relative z-10 mx-auto max-w-7xl px-6 py-12 lg:px-10"><div className="library-state border-rose-400/20 text-rose-200">{message || '系列配音控制台暫時無法使用。'}</div></main>
  }

  return (
    <main className="relative z-10 mx-auto max-w-7xl px-6 py-12 lg:px-10">
      <section className="rounded-3xl border border-white/[.07] bg-white/[.018] p-5 sm:p-7">
        <p className="text-xs font-semibold uppercase tracking-[.22em] text-amber-200/70">Series cast</p>
        <h1 className="mt-2 font-serif text-3xl text-stone-100">多角色系列配音</h1>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-stone-500">同一系列固定旁白與角色聲線。先逐章校正、鎖定劇本，再建立整批 staged 音訊；任何一冊失敗都不會偷換目前版本。</p>

        <form className="mt-6 grid gap-3 border-t border-white/[.06] pt-5 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] sm:items-end" onSubmit={createSeries}>
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-[18rem_minmax(0,1fr)]">
        <aside className="rounded-3xl border border-white/[.07] bg-white/[.018] p-4">
          <h2 className="font-serif text-xl text-stone-100">我的系列</h2>
          <div className="mt-3 space-y-2">
            {series.map((item) => <button className={`w-full rounded-xl border p-3 text-left text-sm transition ${item.id === selectedSeriesId ? 'border-amber-300/30 bg-amber-300/[.08] text-amber-100' : 'border-white/[.06] bg-black/10 text-stone-400 hover:text-stone-200'}`} key={item.id} onClick={() => setSelectedSeriesId(item.id)} type="button"><span className="block truncate">{item.name}</span><span className="mt-1 block text-xs opacity-70">{item.bookCount} 冊 · {item.characterCount} 角色</span></button>)}
        ...
        <section>
          ...
          {details && (
            <>
              <div className="rounded-3xl border border-white/[.07] bg-white/[.018] p-5 sm:p-7">
                <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs text-stone-500">系列</p><h2 className="mt-1 font-serif text-3xl text-stone-100">{details.name}</h2><p className="mt-2 text-sm text-stone-500">旁白：{voiceLabel(details.narratorVoice, voiceOptions)} · 對白間隔 {details.defaultSpeakerPauseMs}ms</p></div><button className="secondary-button px-3 py-2 text-xs" onClick={() => previewVoice(details.narratorVoice)} type="button">播放固定示範句</button></div>

                <section className="mt-6 border-t border-white/[.06] pt-5" aria-label="系列書籍"><h3 className="font-serif text-xl text-stone-100">系列書籍</h3><div className="mt-3 space-y-2">{details.books.slice().sort((left, right) => left.sortOrder - right.sortOrder).map((member) => <div className="rounded-xl border border-white/[.06] bg-black/10 p-3" key={member.id}><p className="text-sm text-stone-200">{member.volumeLabel} · {member.bookTitle}</p><p className="mt-1 text-xs text-stone-500">membership revision {member.membershipRevision}</p></div>)}{details.books.length === 0 && <p className="text-sm text-stone-500">尚未加入正文書籍。</p>}</div>
                ...
                <section className="mt-7 border-t border-white/[.06] pt-5" aria-label="固定角色聲線"><h3 className="font-serif text-xl text-stone-100">固定角色聲線</h3><p className="mt-1 text-sm text-stone-500">角色與別名都限制在這個 owner 的系列；跨角色 alias 衝突會直接拒絕。</p><div className="mt-3 space-y-2">{details.characters.map((character) => <div className="rounded-xl border border-white/[.06] bg-black/10 p-3" key={character.id}><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm text-stone-200">{character.canonicalName} · {character.role}</p><button className="text-xs text-amber-200/80" onClick={() => previewVoice(character.voice)} type="button">播放固定示範句</button></div><p className="mt-1 text-xs text-stone-500">{voiceLabel(character.voice, voiceOptions)} · 別名：{character.aliases.map((item) => item.value).join('、') || '—'}</p></div>)}</div>
              ...
              {batch && <section className="mt-8 rounded-3xl border border-white/[.07] bg-white/[.018] p-5 sm:p-7" aria-label="staged rebuild 狀態"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[.22em] text-amber-200/70">Staged rebuild</p><h2 className="mt-1 font-serif text-2xl text-stone-100">{batch.status}</h2></div>{batch.status === 'ReadyToActivate' && <button className="secondary-button" onClick={() => setActivateDialogOpen(true)} type="button">人工啟用完整系列音訊</button>}</div><ul className="mt-4 space-y-2">{batch.members.map((member) => <li className="flex items-center justify-between gap-3 rounded-xl border border-white/[.06] bg-black/10 p-3 text-sm" key={member.id}><span className="text-stone-300">{details.books.find((book) => book.bookId === member.bookId)?.bookTitle ?? '系列書籍已變更'}</span><span className="text-xs text-stone-500">{member.status}</span></li>)}</ul></section>}
            </>
          )}
        </section>
      </section>
